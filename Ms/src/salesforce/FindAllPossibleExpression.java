...
public class FindAllPossibleExpression {
	// Utility function to evaluate a simple expression 
	    ...
	    static List<Integer> evaluateAll(String expr, 
	                                    int low, int high)  
	    { 
	        ...
	        {  
	            ...
	            List<Integer> l = evaluateAll(expr, low, i - 1); 
	  
	            // r refers to all the possible values 
	            // in the right of operator 'expr[i]' 
	            List<Integer> r = evaluateAll(expr, i + 1, high); 
	    ...
	    static void  findExpress(String expr)
	    {
	    	System.out.println(evaluateAll(expr, 0, expr.length()));
	    }
	    // Driver program 
